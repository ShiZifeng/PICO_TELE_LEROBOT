...
def create_camera_instance(cam_meta: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        if cam_type == "OpenCV":
            if cam_meta.get("readable") is False:
                logger.warning(f"Skipping OpenCV camera {cam_id}: device opens but does not return frames.")
                return None
...
def save_images_from_all_cameras(
    ...
    if camera_type is None:
        logger.info(
            "No camera type filter provided; listed cameras only. "
            "Run `python -m lerobot.find_cameras opencv` or `python -m lerobot.find_cameras orbbec` "
            "to capture test images for one backend."
        )
        return
