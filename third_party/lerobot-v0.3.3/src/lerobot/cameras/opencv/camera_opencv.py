...
class OpenCVCamera(Camera):
    ...
    def _configure_capture_settings(self) -> None:
        ...
        if self.config.fourcc is not None:
            ...
            actual_fourcc_str = "".join(chr((actual_fourcc >> 8 * i) & 0xFF) for i in range(4)).strip()
            if not success:
                logger.warning("%s failed to set fourcc=%s.", self, self.config.fourcc)
            else:
                logger.info("%s set fourcc=%s (actual=%s).", self, self.config.fourcc, actual_fourcc_str)
    ...
    def find_cameras() -> list[dict[str, Any]]:
        ...
        for target in targets_to_scan:
            ...
            if camera.isOpened():
                ...
                readable = False
                readable_width = None
                readable_height = None
                for _ in range(5):
                    ret, frame = camera.read()
                    if ret and frame is not None:
                        readable = True
                        readable_height, readable_width = frame.shape[:2]
                        break
                camera_info = {
                ...
                if readable:
                    camera_info["readable_stream_profile"] = {
