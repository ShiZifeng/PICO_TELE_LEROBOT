...
if [[ "$1" == "--conda" ]]; then
    ...
    if [[ "$(printf '%s\n' "3.10" "$PYTHON_MAJOR_MINOR" | sort -V | head -n1)" != "3.10" ]]; then
        echo "LeRobot requires Python >= 3.10; using Python 3.10 for the conda environment."
...
# Check if the --install parameter is passed
    ...
    if [[ -d "$LEROBOT_PATH" ]]; then
        ...
        uv pip install -e "$LEROBOT_PATH" || { echo "Failed to install lerobot from $LEROBOT_PATH"; exit 1; }
    else
